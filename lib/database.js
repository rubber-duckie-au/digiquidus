var mongoose = require('mongoose')
  ...
  , hitbtc = require('./markets/hitbtc')
  , whitebit = require('./markets/whitebit');
...
function get_market_data(market, cb) {
  switch(market) {
    case 'bittrex':
      ...
      break;
    case 'hitbtc':
      hitbtc.get_data(settings.markets.coin, settings.markets.exchange, function (err, obj) {
        return cb(err, obj);
      });
      break;
    case 'whitebit':
      whitebit.get_data(settings.markets.coin, settings.markets.exchange, function(err, obj){
        return cb(err, obj);
      });
      break;
    case 'bleutrade':
...
module.exports = {
  ...
  update_richlist: function(list, cb){
    if(list == 'received') {
      ...
      Address.find({}).sort({balance: 'desc'}).limit(103).exec(function(err, addresses){
        Richlist.update({coin: settings.coin}, {
          balance: addresses,
        }, function() {
          return cb();
  ...
  get_distribution: function(richlist, stats, cb){
    ...
    lib.syncLoop(richlist.balance.length, function (loop) {
      ...
      if (count <= 28 ) {
      	if (richlist.balance[i].a_id != 'dMsop93F7hbLSA2d666tSPjB2NXSAfXpeU' && richlist.balance[i].a_id !='daigDQ7VxAFwmhh59HstA53KYD5a4q81N5' && richlist.balance[i].a_id != 'dVibZ11CVyiso4Kw3ZLAHp7Wn77dXuvq1d') {
